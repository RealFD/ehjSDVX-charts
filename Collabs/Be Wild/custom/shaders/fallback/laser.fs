...
{    
    ...
    if (uIndex == 0)
    {
        laserColor = l_color;
        if (laserPart == 0){
            mainColor = texture(l_middle, vec2(x,fsTex.y));
        }
        if (laserPart == 1){
            mainColor = texture(l_entry, vec2(x,fsTex.y));
        }
        if (laserPart == 2){
            mainColor = texture(l_exit, vec2(x,fsTex.y));
        }
    } 
    else
    {
        laserColor = r_color;
        if (laserPart == 0){
            mainColor = texture(r_middle, vec2(x,fsTex.y));
        }
        if (laserPart == 1){
            mainColor = texture(r_entry, vec2(x,fsTex.y));
        }
        if (laserPart == 2){
            mainColor = texture(r_exit, vec2(x,fsTex.y));
        }
    }



    target = mainColor * laserColor;
