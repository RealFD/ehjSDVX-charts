	...
	loaders = {
		...
		function(modname)
			...
			for path in (string.gfind or string.gmatch)(xero.package.path, '[^;]+') do
				...
				if not true then --GAMESTATE:GetFileStructure(filepath) then --TODOs check
...
function methods:next()
	...
	if swap.n == 0 then
		xero.stable_sort(stage, self.reverse_comparator) --TODOs was without xero.
	end
	if stage.n == 0 then
		if list.n == 0 then
			while swap.n ~= 0 do
